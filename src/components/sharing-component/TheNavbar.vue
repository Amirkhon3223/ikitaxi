<template>
  <div class="flex flex-col">
    ...
    <!-- Navbar -->
    <nav class="py-4 bg-secondary backdrop-blur-md w-full fixed top-0 left-0 right-0 z-10">
      <div class="flex justify-between max-w-7xl mx-auto z-10 px-3">
        <!-- Logo Container -->
        <div class="flex items-center">
          <!-- Logo -->
          <router-link to="/" class="cursor-pointer flex items-center" @click="scrollToTop()">
        ...
        <div class="items-center hidden space-x-14 lg:flex">
          <router-link v-for="link of links" :to="link.link" @click="scrollToTop()"
            class="flex text-lg font-semibold  transition-all cursor-pointer text-white ">
            {{ $t(link.name) }}
          </router-link>
        </div>
        <the-dropdown-menu />
      ...
      <!-- Mobile Menu -->
      <div v-if="isMobileMenuOpen" class="lg:hidden bg-secondary w-full px-4 py-2">
        <router-link v-for="link of links" :to="link.link" @click="scrollToTop()"
          class="block text-lg font-semibold text-center transition-all cursor-pointer mb-4 text-white">
          {{ $t(link.name) }}
        </router-link>
      </div>
...
const links = [
  ...
  { name: "navbar.links.1.name", link: 'driver' },
  { name: "navbar.links.2.name", link: 'about' },
];
</script>
