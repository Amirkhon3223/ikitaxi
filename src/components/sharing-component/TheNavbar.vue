<template>
  <div class="flex flex-col">
    ...
    <nav class="py-2 bg-secondary backdrop-blur-md w-full fixed top-0 left-0 right-0 z-10">
      <div class="flex justify-between max-w-7xl mx-auto z-10 px-3 items-center">
        ...
        <div class="lg:hidden flex items-center justify-center">
          <button class="focus:outline-none" @click="toggleMobileMenu">
            <svg v-if="!isMobileMenuOpen" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round"
                 stroke-linejoin="round" class="w-10 h-10 transform transition-transform stroke-primary">
              <path d="M3 12h18M3 6h18M3 18h18"></path>
            </svg>
            <svg v-if="isMobileMenuOpen" viewBox="0 0 24 24"
                 class="w-10 h-10 transform transition-transform stroke-primary">
        ...
        <!-- Links Section -->
        <div class="items-center hidden space-x-14 lg:flex">
          <router-link
              to="/"
              class="flex text-md font-normal  transition-all cursor-pointer
              text-gray-100 hover:text-[20px] hover:text-white">
          ...
          <router-link
              ...
              :to="'#' + link.scrollId"
              @click="scrollToSection(link.scrollId)"
              class="flex text-md font-normal  transition-all cursor-pointer
          text-gray-100 hover:text-[20px] hover:text-white">
            {{ $t(link.name) }}
          </router-link>
        ...
        <div class="flex items-center">
          <router-link to="driver">
            <button
                @click="scrollToTop()"
                class="hidden sm:block text-white text-lg bg-primary rounded-full px-8 h-11 item-center">
              {{ $t("navbar.links.4.name") }}
            </button>
          </router-link>
          <the-dropdown-menu/>
        </div>
      ...
      <!-- Mobile Menu -->
      <div v-if="isMobileMenuOpen" class="lg:hidden text-center bg-secondary w-full px-4 space-y-5 py-2">
        <a
            href="/"
            @click="toggleMobileMenu"
            class="flex flex-col text-center text-white text-lg font-normal transition-all">
          {{ $t("navbar.links.0.name") }}
        </a>
        ...
        <router-link
            ...
            :to="'#' + link.scrollId"
            @click="scrollToSection(link.scrollId)"
            class="flex flex-col text-lg text-center text-white font-normal transition-all">
          {{ $t(link.name) }}
        </router-link>
        ...
        <router-link to="driver">
          <button
              @click="scrollToTop()"
              class="text-white text-lg bg-primary rounded-full px-8 h-11 mt-4">
            {{ $t("navbar.links.4.name") }}
          </button>
        </router-link>
...
/* eslint-disable */
import TheDropdownMenu from '@/components/sharing-component/TheDropdownMenu.vue'
import {ref} from 'vue';
...
//dropdown для смены языка
const toggleMobileMenu = () => {
  isMobileMenuOpen.value = !isMobileMenuOpen.value;
};

// Берем данные из json локализации название навигации с переводами
const links = [
  {name: "navbar.links.0.name", link: "/"},
  {name: "navbar.links.1.name", scrollId: "services"},
  {name: "navbar.links.2.name", scrollId: "advantage"},
  {name: "navbar.links.3.name", scrollId: "tariffs"},
];
</script>
