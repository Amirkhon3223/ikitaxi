  ...
  <div class="flex flex-col">
    ...
    <nav class="py-4 bg-secondary backdrop-blur-md w-full fixed top-0 left-0 right-0 z-10">
      <div class="flex justify-between max-w-7xl mx-auto z-10 px-3">
        ...
        <div class="lg:hidden flex items-center justify-center">
          <button class="focus:outline-none" @click="toggleMobileMenu">
            <svg v-if="!isMobileMenuOpen" viewBox="0 0 24 24" stroke-width="2"
        ...
        <div class="items-center hidden space-x-14 lg:flex">
          <router-link
              v-for="link of links" :to="link.link"
              class="flex text-lg font-semibold  transition-all cursor-pointer text-white ">
            {{ $t(link.name) }}
          </router-link>
        </div>

        <button @click="changeLanguage" class="text-white font-semibold cursor-pointer">
      ...
      <div v-if="isMobileMenuOpen"
           class="lg:hidden bg-secondary w-full px-4 py-2">
        <router-link v-for="link of links" :to="link.link"
...
export default {
  ...
  setup() {
    ...
    const {t, locale} = useI18n({useScope: "global"}); // Получение метода $t и текущего языка
